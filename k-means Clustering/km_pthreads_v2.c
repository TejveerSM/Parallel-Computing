...
void *cluster_assignment(void *assg_args) {

	struct cluster_assg_args *my_args = (struct cluster_assg_args*) assg_args;

	int i, k;
	int thread_id = (*my_args).thread_id;
	int rows = (*my_args).rows;
	int rows_start = thread_id * rows;
        double **row_pointer = (*my_args).row_pointer;	

	int rows_end = rows_start + rows; 
	if(rows_end >= rows_test)
		rows_end = rows_test;
	...
	for(i = rows_start; i < rows_end; i++) {
		...
		for(k = 0; k < clusters; k++) {
			double dist = distance(row_pointer[i], cluster_centroids[k]);		
			if(dist < min) {
				min = dist;
				cluster_id = k;
			}
		...
		sum(local_cluster_centroids[thread_id][cluster_id], row_pointer[i]);
		local_cluster_counts[thread_id][cluster_id] += 1;
...
int main(int argc, char *argv[]) {
	...
	while(iter_count < ITERATIONS) {
		...
		for(i = 0; i < p; i++) {
			struct cluster_assg_args *cluster_args;
			cluster_args = (struct cluster_assg_args *) malloc(sizeof(struct cluster_assg_args));
			(*cluster_args).thread_id = i;
			(*cluster_args).rows = chunks;
			(*cluster_args).row_pointer = mat;

			pthread_create(&p_threads[i], &attr, cluster_assignment, cluster_args);
		}
